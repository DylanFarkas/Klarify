      ...
      <div className="relative overflow-hidden rounded-2xl border border-border bg-surface-muted/60 p-8 animate-[fadeIn_0.4s_ease-out]">
        ...
        <div className="relative z-10">
          ...
          <p className="max-w-2xl text-lg leading-relaxed text-muted">
            Con el backlog estimado del Agente 3, este paso ordena épicas e historias
            según valor de negocio y esfuerzo usando la metodología MoSCoW. El equipo
            revisa y ajusta antes de consolidar.
          </p>

          {hasInput && Object.keys(state.priorities).length > 0 && (
            <div className="mt-5 flex flex-wrap items-center gap-5 animate-[fadeIn_0.5s_ease-out_0.2s_both]">
              ...
              <span className="flex items-center gap-1.5 text-sm text-subtle">
                ...
                <span className="font-medium text-foreground">{Object.keys(state.priorities).length}</span>
                <span>priorizada{Object.keys(state.priorities).length !== 1 ? 's' : ''}</span>
              </span>
              {isApproved && (
            ...
            <div className="relative overflow-hidden rounded-2xl border border-success/25 bg-linear-to-br from-success/[0.07] via-success/3 to-transparent px-6 py-6 animate-[scaleIn_0.35s_ease-out]">
              ...
              <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-sm text-success/70">
                      {epicCount} épica{epicCount !== 1 ? 's' : ''} · {storyCount} historia{storyCount !== 1 ? 's' : ''} priorizada{storyCount !== 1 ? 's' : ''}.
                    </p>
                    <div className="mt-1.5 flex flex-wrap gap-2">
                      {Object.entries(categoryCounts).map(([cat, count]) => (
                        <span
                          key={cat}
                          className="inline-flex items-center gap-1 rounded-full border border-success/20 bg-success/10 px-2.5 py-0.5 text-[10px] font-bold text-success/80"
                        >
                          {getFrameworkLabels(state.framework)[cat]}
                ...
                <div className="flex shrink-0 items-center sm:ml-auto">
                  <button
                    disabled
                    className="rounded-xl bg-muted/60 px-5 py-2.5 text-sm font-bold text-muted-foreground opacity-60 cursor-not-allowed border border-border"
                  >
