...
import {
  buildHarnessToolDeclarations,
  parseHarnessToolArgs,
} from '@/lib/schemas/harness-tools';
...
export async function executeHarnessTool(
  ...
  const parsed = parseHarnessToolArgs(name, rawArgs ?? {});
  if (!parsed.success) {
    return toolError(parsed.error, 'INVALID_ARGS');
  }
