...
export function TranscriptionPanel({ transcription }: TranscriptionPanelProps) {
  ...
  if (!transcription) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-6 backdrop-blur-sm dark:border-white/10 dark:bg-white/[0.03]">
        <h3 className="mb-4 text-lg font-bold text-slate-900 dark:text-white">Transcripción</h3>
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-white/10">
        <div className="flex items-center gap-3">
          <h3 id="transcription-heading" className="text-lg font-bold text-slate-900 dark:text-white">
            Transcripción
          </h3>
          {transcription.duration > 0 && (
            <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-500 dark:bg-white/10 dark:text-white/60">
        ...
        {showSegments ? (
          ...
          <div className="flex flex-col gap-3">
            {transcription.segments.map((segment, index) => (
              <div
                key={index}
                className="group flex gap-3 rounded-xl p-3 transition-colors hover:bg-slate-50 dark:hover:bg-white/[0.03]"
                style={{ animationDelay: `${index * 50}ms` }}
              >
                {/* Timestamp */}
                {segment.end > 0 && (
                  <span className="shrink-0 pt-0.5 text-xs font-mono text-slate-400 dark:text-white/30">
                  ...
                  {segment.speaker && (
                    <span
                      className={[
                        ...
                        segment.speaker === 'Cliente' ? 'text-[#005BBF]' : 'text-slate-500 dark:text-white/50',
                      ].join(' ')}
                    >
                      {segment.speaker}
                    </span>
          ...
          <div className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700 dark:text-white/80">
