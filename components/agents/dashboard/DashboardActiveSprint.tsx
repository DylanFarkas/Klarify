...
export function DashboardActiveSprint({
	...
	if (!activeSprint) {
		return (
			<section className="rounded-xl border border-border bg-surface px-4 py-5 md:px-5">
				...
				<Link
					...
					className="mt-4 inline-flex items-center rounded-lg border border-border px-3 py-1.5 text-[12px] font-medium text-foreground transition-colors hover:bg-surface-hover"
				>
					Ir al backlog
				</Link>
			</section>
	...
	return (
		<section className="flex flex-col gap-3" aria-label="Sprint activo">
			<div className="flex flex-wrap items-start justify-between gap-3">
				...
				<div className="flex flex-wrap items-center gap-2">
					{executionBoardEnabled ? (
						<Link
							href="/agentes/board"
							className="inline-flex items-center rounded-lg border border-border px-3 py-1.5 text-[12px] font-medium text-muted transition-colors hover:bg-surface-hover hover:text-foreground"
						>
							Ir al tablero
						</Link>
					) : null}
			...
			<div className="overflow-hidden rounded-xl border border-border bg-surface">
				<div className="overflow-x-auto">
					<table className="w-full min-w-120 text-left text-[12px]">
						<thead>
							<tr className="border-b border-border bg-surface-hover/40 text-[11px] text-subtle">
								<th className="px-4 py-2.5 font-medium">Historia</th>
								<th className="px-4 py-2.5 font-medium">Épica</th>
								<th className="hidden px-4 py-2.5 font-medium sm:table-cell">Estimación</th>
						...
						<tbody>
							{stories.length === 0 ? (
								...
								stories.map((entry) => (
									<tr key={entry.story.id} className="border-b border-border last:border-b-0">
										<td className="px-4 py-2.5">
											<p className="font-medium text-foreground">{entry.story.title}</p>
											<p className="mt-0.5 text-[11px] text-subtle">{entry.story.id}</p>
										</td>
										<td className="px-4 py-2.5 text-muted">{entry.epicTitle}</td>
